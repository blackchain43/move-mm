...
import { COIN_TYPE_SUI, COIN_TYPE_TOKEN, WS } from "../../utils/constants";
...
const Home = () => {
  ...
  const [coinType, setCoinType] = React.useState(COIN_TYPE_SUI);
  ...
  useQuery(
    ...
    () =>
      systemService
        .fetchToken({ address: wallet?.address })
        .then(async (result) => {
  ...
  const {
    ...
    ["systemService.fetchWallets", { address: wallet?.address }],
    ({ queryKey }) => {
  ...
  const {
    ...
    ["systemService.swapState", { address: wallet?.address }],
    ({ queryKey }) => {
  ...
  const onFilter = React.useCallback(async () => {
    ...
    const columm = type === COIN_TYPE_SUI ? "suiBalance" : "tokenBalance";
                ...
                <Table
                  ...
                  columns={[
                    ...
                    },
                    {
                      title: "SUI",
                      key: "SUI",
                      dataIndex: "suiBalance",
                      render: (sui) => <span>{formatMoney(sui)}</span>,
                    },
                    {
                      title: "TOKEN",
                      key: "TOKEN",
