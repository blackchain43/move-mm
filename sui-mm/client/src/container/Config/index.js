...
import { useWallet as connectWallet } from "@suiet/wallet-kit";
...
const Config = () => {
  ...
  const {
    ...
    ["systemService.fetchConfig", { requestor: wallet?.address || "" }],
    ({ queryKey }) => {
  ...
  const addConfig = React.useCallback(() => {
    setSelectedRow(null);
    form.setFieldsValue({
      tokenAType: COIN_TYPE_TOKEN,
      tokenBType: COIN_TYPE_SUI,
      swapAforB: true,
      slippage: 0.1,
  ...
  const onChangeType = React.useCallback((type, value) => {
    if (type === "A") {
      if (value === COIN_TYPE_SUI) {
        form.setFieldValue("decimalsA", 9);
        form.setFieldValue("tokenBType", COIN_TYPE_TOKEN);
        form.setFieldValue("decimalsB", DECIMAL_NUMBER);
      } else {
        form.setFieldValue("decimalsA", DECIMAL_NUMBER);
        form.setFieldValue("tokenBType", COIN_TYPE_SUI);
        form.setFieldValue("decimalsB", 9);
      }
    } else {
      if (value === COIN_TYPE_SUI) {
        form.setFieldValue("decimalsB", 9);
        form.setFieldValue("tokenAType", COIN_TYPE_TOKEN);
        form.setFieldValue("decimalsA", DECIMAL_NUMBER);
      } else {
        form.setFieldValue("decimalsB", DECIMAL_NUMBER);
        form.setFieldValue("tokenAType", COIN_TYPE_SUI);
        form.setFieldValue("decimalsA", 9);
      }
    }
  }, []);
